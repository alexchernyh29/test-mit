<script setup>
// readme https://github.com/arsendemirci/ag-select & https://arsendemirci-ag-select.netlify.app/
  ...
  >
	<div class="current" @click="onWrapperClick">
	  <span
		...
		@click="onWrapperClick"
		>{{ selectedText }}</span
	  ><i @click.stop="onClear" class="clear" v-if="state.selectedOption && !hiddenClear"
		>&times;</i
	  >
	</div>
...
.select-wrapper {
  ...
  user-select: none;
  white-space: nowrap;
  background: rgba(rgb(168, 85, 247), .1);
	border: none;
	border-radius: 2em;
	padding: 0.5em 1em;
	font-size: .8em;
  ...
  &.open {
	...
	box-shadow: 0 0 10px rgb(168, 85, 247, .4);
  ...
  .list-wrapper {
	...
	.search {
	  ...
	  input {
		...
		border-radius: 4px;
		height: 25px;
		box-shadow: 0 0 2px rgb(168, 85, 247, .7);
		outline: none;
		transition: all 0.22s cubic-bezier(0.5, 0, 0, 1.25);
		&:focus {
		  box-shadow: 0 0 5px rgb(168, 85, 247, .5);
		}
	  }
	}
  }
  ...
  .option {
	...
	&:not(.disabled):not(.selected):hover {
	  background-color: rgb(168, 85, 247, .7);
	  padding-left: 14px;
